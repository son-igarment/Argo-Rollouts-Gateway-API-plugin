...
This plugin, developed by **Phạm Lê Ngọc Sơn**, integrates the [Gateway API](https://gateway-api.sigs.k8s.io/) with Argo Rollouts, making it much easier as any traffic provider that implements the API will automatically be supported by Argo Rollouts.
